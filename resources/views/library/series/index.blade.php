...
<div class="lib-page container-fluid py-3 px-lg-4 px-3">
    ...
    @if($hero && $hero->backdrop)
        <div class="lib-hero lib-hero-series mb-4" style="background-image:url('https://image.tmdb.org/t/p/w1280/{{ $hero->backdrop }}')">
            <div class="lib-hero-overlay"></div>
            <div class="lib-hero-content position-relative z-2">
                <span class="lib-hero-badge lib-hero-badge-series"><i class="bi bi-tv me-1"></i> SERIES LIBRARY</span>
                ...
                <p class="lib-hero-sub">
                    ...
                    @if($hero->year)
                        &bull; {{ $hero->year }}
                    @endif
                    &bull; {{ $series->total() }} titles available
                </p>
                <a href="{{ route('library.series.show', ['tmdbid' => $hero->tmdbid, 'slug' => $hero->slug]) }}" class="lib-btn-hero">
    ...
    <div class="lib-header mb-4">
        ...
        <div class="lib-search mt-3">
            <form method="GET" action="{{ route('library.series.index') }}" class="lib-search-form">
                <i class="bi bi-search lib-search-icon"></i>
                <input type="text" name="q" value="{{ $query ?? '' }}" placeholder="Search your series library..." class="lib-search-input">
                @if(!empty($query))
                    <a href="{{ route('library.series.index') }}" class="lib-search-clear" title="Clear search"><i class="bi bi-x-lg"></i></a>
                @endif
        ...
        @if(!empty($query))
            <p class="lib-search-result-text mt-3 mb-0">
                ...
                <span class="text-muted">&middot; {{ $series->total() }} found</span>
            </p>
        @endif
    </div>
    ...
    <div class="row g-3">
        @forelse($series as $i => $item)
            <div class="col-6 col-sm-4 col-md-3 col-lg-2 lib-card-col" style="--i:{{ $i }}">
                <a href="{{ route('library.series.show', ['tmdbid' => $item->tmdbid, 'slug' => $item->slug]) }}" class="text-decoration-none d-block">
                    <div class="lib-card">
                        <div class="lib-card-poster">
                            <img src="{{ $item->poster ? 'https://image.tmdb.org/t/p/w500' . $item->poster : asset('images/no-poster.png') }}" alt="{{ $item->title }}" loading="lazy">
                            @if($item->rating)
                                <span class="lib-badge lib-badge-rating"><i class="bi bi-star-fill"></i> {{ number_format($item->rating, 1) }}</span>
                            @endif
                            @if($item->year)
                                <span class="lib-badge lib-badge-year">{{ $item->year }}</span>
                            @endif
                            @if(!empty($item->max_seeders))
                                <span class="lib-badge lib-badge-seeders"><i class="bi bi-arrow-up-circle-fill"></i> {{ $item->max_seeders }}</span>
                            @endif
                        ...
                        <div class="lib-card-meta">
                            <h5 class="lib-card-title">{{ Str::limit($item->title, 35) }}</h5>
                            <div class="lib-card-info">
                                @if($item->year)
                                    <span>{{ $item->year }}</span>
                                @endif
                                @if($item->max_seeders)
                                    <span><i class="bi bi-arrow-up-circle-fill"></i> {{ $item->max_seeders }} seeders</span>
                                @endif
                            </div>
                        </div>
                    </div>
...
<style>
/* ══════════════════════════════════════════════════════════════
   LIBRARY SERIES — Premium Dark Theme
   ══════════════════════════════════════════════════════════════ */
