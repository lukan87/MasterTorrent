...
<div class="ms-page">
    ...
    <div class="ms-header">
        <div>
            ...
            <p class="ms-subtitle">
                Pick movies to add. Titles you already own show at the top — everything else is ready to add, plus suggested picks below.
            </p>
        </div>
        <div class="ms-header-badges">
            @if($existingMovies->isNotEmpty())
                <span class="ms-badge ms-badge--green"><i class="bi bi-check2-circle"></i> <b>{{ $existingMovies->count() }}</b> owned</span>
            @endif
            <span class="ms-badge ms-badge--gold"><i class="bi bi-shuffle"></i> <b>{{ $similarMovies->count() }}</b> similar</span>
            <span class="ms-badge ms-badge--accent"><i class="bi bi-magic"></i> <b>{{ $recommendedMovies->count() }}</b> recommended</span>
            <span class="ms-badge"><i class="bi bi-collection"></i> <b>{{ $newMovies->count() }}</b> to add</span>
        </div>
    </div>
    ...
    <form action="{{ route('movies.bulkSelect') }}" method="POST" id="msForm">
        @csrf
...
<section class="ms-section" id="existingSection">
                <div class="ms-section-head">
                    <h2 class="ms-section-title">
                        ...
                        Already in Database
                        <span class="ms-section-count">{{ $existingMovies->count() }}</span>
                    </h2>
                </div>
                <div class="ms-inline">
                    @forelse($existingMovies as $movie)
                        @if(!empty($movie['poster_path']))
                            <div class="ms-card is-existing">
                                <div class="ms-poster">
                                    <img src="https://image.tmdb.org/t/p/w300_and_h450_bestv2{{ $movie['poster_path'] }}" alt="{{ $movie['title'] }}" loading="lazy">
                                    <div class="ms-poster-fade"></div>
                                    <span class="ms-owned-chip dim"><i class="bi bi-check2-circle"></i> Owned</span>
                                </div>
                                <div class="ms-card-body">
                                    <p class="ms-card-title">{{ $movie['title'] }}</p>
                                    <p class="ms-card-overview">{{ $movie['overview'] ?: '' }}</p>
                                </div>
                            </div>
                        @endif
                    @empty
            ...
            {{-- ===== ROW B: New movies to add (horizontal scroll when more than 8) ===== --}}
<section class="ms-section" id="newSection">
                <div class="ms-section-head">
                    <h2 class="ms-section-title">
                        <span class="ms-ic ms-ic--accent"><i class="bi bi-plus-circle"></i></span>
                        New Movies to Add
                        <span class="ms-section-count">{{ $newMovies->count() }}</span>
                    </h2>
                ...
                @endif
                <div class="ms-grid {{ $newMovies->count() > 8 ? 'ms-grid--locked' : '' }}">
                    @forelse($newMovies as $movie)
                        @if(!empty($movie['poster_path']))
                            <div class="ms-card ms-card--addable" data-id="{{ $movie['id'] }}" data-title="{{ $movie['title'] }}">
                                <div class="ms-poster">
                                    <img src="https://image.tmdb.org/t/p/w300_and_h450_bestv2{{ $movie['poster_path'] }}" alt="{{ $movie['title'] }}" loading="lazy">
                                    <div class="ms-poster-fade"></div>
                                    @if(isset($movie['vote_average']) && $movie['vote_average'])
                                        <span class="ms-pill ms-pill--rating"><i class="bi bi-star-fill"></i>{{ number_format($movie['vote_average'], 1) }}</span>
                                    @endif
                                    @if(!empty($movie['release_date']))
                                        <span class="ms-pill ms-pill--year"><i class="bi bi-calendar3"></i>{{ substr($movie['release_date'], 0, 4) }}</span>
                                    @endif
                                </div>
                                <div class="ms-card-body">
                                    <p class="ms-card-title">{{ $movie['title'] }}</p>
                                    <p class="ms-card-overview">{{ $movie['overview'] ?: 'No overview available.' }}</p>
                                    <label class="ms-add">
                                        <input type="checkbox" name="movies[]" value="{{ $movie['id'] }}" class="ms-check">
                    ...
                    @empty
                        <div class="ms-empty" style="grid-column:1/-1">
                            ...
                            No new movies to add — everything from this search is already in your database.
                        </div>
                    @endforelse
                </div>
            </section>
            ...
            <hr class="ms-section-divider">
{{-- ===== ROW C: Similar movies (based on the searched title) ===== --}}
            <section class="ms-section" id="similarSection">
                <div class="ms-section-head">
                    <h2 class="ms-section-title">
                        <span class="ms-ic ms-ic--gold"><i class="bi bi-shuffle"></i></span>
                        Similar Movies
                        <span class="ms-section-count">{{ $similarMovies->count() }}</span>
                    </h2>
                ...
                @if($similarMovies->isNotEmpty())
                    <div class="ms-scroll">
                        @foreach($similarMovies as $entry)
                            @php $m = $entry['movie']; @endphp
                            @if(!empty($m['poster_path']))
                                <div class="ms-card ms-card--addable" data-id="{{ $m['id'] }}">
                                    <div class="ms-poster">
                                        <img src="https://image.tmdb.org/t/p/w300_and_h450_bestv2{{ $m['poster_path'] }}" alt="{{ $m['title'] }}" loading="lazy">
                                        <div class="ms-poster-fade"></div>
                                        @if(isset($m['vote_average']) && $m['vote_average'])
                                            <span class="ms-pill ms-pill--rating"><i class="bi bi-star-fill"></i>{{ number_format($m['vote_average'], 1) }}</span>
                                        @endif
                                        @if(!empty($m['release_date']))
                                            <span class="ms-pill ms-pill--year"><i class="bi bi-calendar3"></i>{{ substr($m['release_date'], 0, 4) }}</span>
                                        @endif
                                        @if($entry['exists'])
                                    ...
                                    <div class="ms-card-body">
                                        <p class="ms-card-title">{{ $m['title'] }}</p>
                                        <p class="ms-card-overview">{{ $m['overview'] ?: 'No overview available.' }}</p>
                                        @if($entry['exists'])
                                            <span class="ms-link"><i class="bi bi-check2-circle"></i> Already owned</span>
                                        @else
                                            <label class="ms-add">
                                                <input type="checkbox" name="movies[]" value="{{ $m['id'] }}" class="ms-check">
                                                <span class="ms-add-label">Add</span>
                                            </label>
                                        @endif
                                    </div>
            ...
            <section class="ms-section" id="recommendedSection">
                ...
                @if($recommendedMovies->isNotEmpty())
                    <div class="ms-scroll">
                        @foreach($recommendedMovies as $entry)
                            @php $m = $entry['movie']; @endphp
                            @if(!empty($m['poster_path']))
                                <div class="ms-card ms-card--addable" data-id="{{ $m['id'] }}">
                                    <div class="ms-poster">
        ...
        @else
            <div class="ms-empty">
                ...
                No movies found for "{{ $query }}". Please try a different search.
            </div>
        @endif
{{-- ===== Sticky footer with live counter ===== --}}
        @if($movies->isNotEmpty())
            <div class="ms-footer">
                ...
                <button type="submit" class="ms-add-btn" id="submitBtn" data-bs-toggle="tooltip" title="Add all checked movies to the database">
                    <i class="bi bi-plus-circle"></i> Add Selected Movies to Database
                </button>
            </div>
        @endif
    </form>
