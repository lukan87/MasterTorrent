 ...
 *     'forced'?: string
 * }
 * @phpstan-type Chapter array{
 *     start: string,
 *     title: string
 * }
 * @phpstan-type Menu array<Chapter>
 */
class MediaInfo
{
     ...
     *     chapters: ?non-empty-list<Chapter>
     * }
     */
        ...
        foreach ($sections as $key => $section) {
            ...
            if (!empty($section)) {
                if ($keySection === 'general') {
                    ...
                    $output[$keySection][] = $this->parseProperty($section, $keySection);
                } elseif ($keySection === 'menu') {
                    $output['chapters'] = $this->parseChapters($section);
                }
            }
        }
     ...
     * @return list<Chapter>
     */
    private function parseChapters(array $lines): array
    {
        $chapters = [];

        foreach ($lines as $line) {
            if (preg_match('/^(\d{2}:\d{2}:\d{2}(?:\.\d{3})?)\s*:\s*(?:[a-z]{2}:)?(.+)$/i', $line, $matches)) {
                $chapters[] = [
                    'start' => $matches[1],
    ...
    {
        return [
            'general'  => empty($data['general']) ? null : $data['general'],
            'video'    => empty($data['video']) ? null : $data['video'],
            'audio'    => empty($data['audio']) ? null : $data['audio'],
            'text'     => empty($data['text']) ? null : $data['text'],
            'chapters' => empty($data['chapters']) ? null : $data['chapters'],
        ];
    }
